...
WSL. CI: PR validation runs `-Ppublish clean verify` (javadoc errors fail there); push to
`main` builds a GPG-signed release and publishes it to Maven Central + a `vX.Y.Z` tag (develop
is the gate only — bump before the release merge; the same version can't publish to Central
twice). `benchmark.yml` is manual-dispatch
